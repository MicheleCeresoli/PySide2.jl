...
@inline function set_animated!(w::QMainWindow, e::Bool) 
    w.obj.setAnimated(e)
    nothing 
end

@inline function set_central_widget!(w::QMainWindow, wdg::QWidget)
    w.obj.setCentralWidget(wdg)  
    nothing 
end

@inline function set_dock_nesting_enabled!(w::QMainWindow, e::Bool)
    w.obj.setDockNestingEnabled(e)    
    nothing 
end

@inline function set_document_mode!(w::QMainWindow, e::Bool)
    w.obj.setDocumentMode(e)
    nothing 
end

@inline function set_icon_size!(w::QMainWindow, qs::QSize)
    w.obj.setIconSize(qs)
    nothing 
end

@inline show!(w::QMainWindow) = show_widget(w)

@inline unified_title_and_toolbar_on_mac!(w::QMainWindow) = pyconvert(Bool, w.obj.unifiedTitleAndToolBarOnMac())
